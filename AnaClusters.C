void AnaClusters(const Int_t proc, const char *particle)
{
  ...
  const char *dir_out = "/gpfs/mnt/gpfs02/phenix/spin/spin1/phnxsp01/zji/data/eic/histos";
  auto f_out = new TFile(Form("%s/clus_%s_theta_%g_%gdeg-%d.root", dir_out, particle, theta_min, theta_max, proc), "RECREATE");
  ...
  for(auto energy : v_energy)
  {
    ...
    Float_t pmc[3][1], edep[ntype][max_track];
